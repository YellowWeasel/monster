...
public class MoneyView extends View {
    ...
    //钱文本 e.g.    789.15
    private float mMoney = 0.00f;
    private String mMoneyText = mMoney + "";
...
//    private int mPointPaddingRight = dp2px(4); //小数点与分的间隔
    private int mPrefixPadding = dp2px(2);//元与前缀的距离
    private int mPointPaddingCent = dp2px(4);//小数点的距离
    private int mUnitPadding = dp2px(4);//单位与价格的距离
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        mMoney = typedArray.getFloat(R.styleable.MoneyView_money, 0.00f);
        mMoneyText = ErayicTextUtil.getPrice(mMoney);
        ...
        mUnitSize = typedArray.getDimensionPixelSize(R.styleable.MoneyView_unit_size, mUnitSize);

        mUnit = typedArray.getString(R.styleable.MoneyView_unit_text);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mPaint.getTextBounds(mUnit, 0, mUnit.length(), mUnitBound);
        //文本占据的宽度
        if (mMoney == 0) {
            mTextWidth = mUnitBound.width() + mPointPaddingCent * 2 + mUnitPadding + mPrefixPadding * 3;
        } else {
            mTextWidth = mYuanBound.width() + mCentBound.width() + mPrefixBound.width() + mPointBound.width() + mUnitBound.width()
                    + mPointPaddingCent + mUnitPadding + mPrefixPadding;
        }
...
//        int drawX = (getMeasuredWidth() - mTextWidth) / 2;//居中
        ...
        if (mMoney == 0) {
            ...
            mPaint.setTextSize(mYuanSize);
            //绘制单位
            drawX += mCentBound.width();
            mPaint.setTextSize(mUnitSize);
            canvas.drawText(mUnit, drawX, drawY, mPaint);
        } else {
            //绘制前缀
            ...
            //绘制小数点
            drawX += mYuanBound.width();
            canvas.drawText(POINT, drawX, drawY, mPaint);
            //绘制分
            drawX += mPointPaddingCent;
            mPaint.setTextSize(mCentSize);
            canvas.drawText(mCent, drawX, drawY, mPaint);
            //绘制单位
            drawX += mCentBound.width() + mUnitPadding;
            mPaint.setTextSize(mUnitSize);
            canvas.drawText(mUnit, drawX, drawY, mPaint);
        }
    }
    ...
    public void setMoney(float price, String unit) {
        mMoney = price;
        mMoneyText = ErayicTextUtil.getPrice(mMoney);
        mUnit = unit;
        requestLayout();
